				       ...
				       size_t *out_num_read)
{
	...
	e = GptFindEntryByName(ctx->gpt, partition_name, NULL);
	if (e == NULL) {
					 ...
					 uint64_t *out_size)
{
	...
	*out_size = GptGetEntrySizeBytes(ctx->gpt, e);
...
static vb2_error_t vb2_load_ramdisk(GptData *gpt, enum GptPartition part,
				    struct vb2_kernel_params *params,
				    vb2ex_disk_handle_t disk_handle,
				    uint32_t *bytes_used)
{
	...
	e = GptFindEntryByName(gpt, GptPartitionNames[part], gpt->current_ab_slot);
	if (!e) {
		VB2_DEBUG("Unable to find %s%s\n", GptPartitionNames[part],
			  gpt->current_ab_slot);
		return VB2_ERROR_LOAD_PARTITION_READ_BODY;
	}
					     ...
					     uint32_t *bytes_used)
{
	...
	ret = vb2_load_ramdisk(gpt, GPT_ANDROID_VENDOR_BOOT, params, disk_handle, bytes_used);
	if (ret != VB2_SUCCESS) {
	...
	ret = vb2_load_ramdisk(gpt, GPT_ANDROID_INIT_BOOT, params, disk_handle, bytes_used);
	if (ret != VB2_SUCCESS) {
	...
	if (!strncmp(partition, "boot", strlen("boot"))) {
		...
		*out_pointer = (uint8_t *)avb_data->params->kernel_buffer;
		...
		return AVB_IO_RESULT_OK;
		   ...
		   !strncmp(partition, "init_boot", strlen("init_boot"))) {
		...
		*out_num_bytes_preloaded = num_bytes;
		if (!strncmp(partition, "vendor_boot", strlen("vendor_boot")))
			*out_pointer = (uint8_t *)avb_data->params->kernel_buffer +
				       avb_data->params->vendor_boot_offset;
		if (!strncmp(partition, "init_boot", strlen("init_boot")))
			*out_pointer = (uint8_t *)avb_data->params->kernel_buffer +
				       avb_data->params->init_boot_offset;

		return AVB_IO_RESULT_OK;
	} else if (!strncmp(partition, "pvmfw", strlen("pvmfw"))) {
