...
uint32_t vb2_nv_get(struct vb2_context *ctx, enum vb2_nv_param param)
{
	...
	case VB2_NV_DEV_ENABLE_FASTBOOT:
	...
	case VB2_NV_OEM_LOCK:
		return GETBIT(VB2_NV_OFFS_DEV, VB2_NV_DEV_FLAG_OEM_LOCK);
		...
		uint32_t value)
{
	...
	case VB2_NV_OEM_LOCK:
		SETBIT(VB2_NV_OFFS_DEV, VB2_NV_DEV_FLAG_OEM_LOCK);
		break;
